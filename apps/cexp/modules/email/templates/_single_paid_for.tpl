  ...
  <div class="post">
    ...
    <h3 class="posted">Sent on: {$smarty.now|date_format:'%I:%M %p - %A %B %e %Y'} </h3>
      ...
      <div class="story">
        <p>
			<pre>
     Package code: <span> {$package.package_code} </span><br />
     Courier name: <span> {$package.name} </span><br />
     From address: <span> {$package.from_text_address|default:"None"} </span><br />
     To address:   <span> {$package.to_text_address|default:'None'} </span><br />
